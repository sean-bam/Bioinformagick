...
#iterate over the columns
 for(my $i=0;$i<$alen;$i++){				# by position
  ...
  #storing new frequencies of all AAs here
  my %fre = ();
  ...
  #assign consensus as "-" by default
  my $caa = "-";
  if($iccut>=0 or $hocut>=0){					# compute frequencies
  
   #set frequency of all AAs as pseudocount * SWISS PROT AA frequency. 
   #pseudocount is 0 by default, so default aa freq is 0.
   foreach my $aa (keys %aafreq){ $fre{$aa} = $pse*$aafreq{$aa};}
   #foreach my $aa (keys %aafreq){print "$aa $fre{$aa} \n";}
   ...
   #get the aligned AA
    ...
    #increase the frequency of the aligned AA by the hh94 score of the sequence
    #this is only aligned AAs, not all AAs
    #print "col is $i, seq is $j, AA is $ch, frequency is $aafreq{$ch} \n";
    if($aafreq{$ch}>0){ $fre{$ch} += $$rswe[$j];}
    
    #if($aafreq{$ch}>0){print "updating frequency to $fre{$ch} \n"}
    
    
    #otherwise, if there is no aligned residue in this sequence+column
    #update the frequency of all 20 AAs using the swiss prot frequency * sequence weight
    #by default, $igno = 0, so we do this, unless -aaonly is added
    elsif($igno==0){foreach my $aa (keys %aafreq){ $fre{$aa} += $aafreq{$aa}*$$rswe[$j];}}# print "col is $i, seq is $j, AA is $aa, swiss-prot freq is $aafreq{$aa}, seq weight is $$rswe[$j], new freq is $fre{$aa}\n"}}
   #foreach my $aa (keys %aafreq){print "col is $i $aa $fre{$aa} \n";}
   }
  }
  	#printf "Pos %d\tG= %.3f\n",$i+1,$nga;
  ...
  if($hocut>=0){						# compute homogeneity & consensus
   print "===HOMO+Consensus==============\n";
   print "col is $i \n";
   
   #best AA is none by default
   my $bestaa = "";
   
   #best weight is -100 * (number of seqs + pseudoseqs) 
   my $bestwt = -100.0*($nseq+$pse);
   
   #something to about ignoring gap chars 
   my $pcnt = $nseq;
   
   #if ignoring gap chars, which we aren't by default
   if($igno>0){
    print "running this";
    $pcnt = 0;
   ...
   foreach my $aa (keys %aafreq){
    ...
    #weight of the AA is 0
    my $wt = 0;
    
    #do an all-versus-all comparison against other AAs.
    #assign weight as the sum of the blosum62 scores * frequency
    foreach my $bb (keys %fre){ $wt += $simmat{$aa.$bb}*$fre{$bb};} #print "AA1 is $aa, AA2 is $bb, frequency of AA2 is $fre{$bb}, blosum62 of $aa$bb is $simmat{$aa.$bb}, updating weight of AA1 to $wt \n"}
    #print "AA1 is $aa, weight is $wt \n";
    
    #if AA weight is below 0, give it a score of 0, assign AA as best AA
    if($wt>$bestwt){ $bestwt = $wt;$bestaa = $aa;}
    #print "AA1 is $aa, weight is $wt, bestwt is $bestwt, BESTAA is $aa \n"
   }
   
   $hom = ($bestwt/($pcnt+$pse) - $expect{$bestaa})/($simmat{$bestaa.$bestaa} - $expect{$bestaa}) if($bestaa ne "" and $bestwt/($nseq+$pse)>$expect{$bestaa});
   print "bestwt is $bestwt, pcnt is $pcnt, pse is $pse, BESTAA is $bestaa, bestAAexpect is $expect{$bestaa}, blosumscore is $simmat{$bestaa.$bestaa}. Thus, homogeneity is $hom\n";
   if($nga<=$nseq*$grcon){
